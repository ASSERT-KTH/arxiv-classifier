...
# https://arxiv.org/help/api/user-manual#53-subject-classifications
CATEGORIES = [
...
def _entries():
    ...
    search_query = r"cat:cs+OR+cat:stat.ML+OR+cat:physics"
    r = requests.get(
        f"http://export.arxiv.org/api/query?search_query={search_query}&sortBy=lastUpdatedDate&max_results=1000"
    )
    r.raise_for_status()
...
entries = _entries()
texts = [
    single_line(entry["title"] + " abstract: " + entry["summary"]) for entry in entries
]
...
for pred, score, text, entry in zip(*entries):
    arxiv_id = entry["id"]
    label = " ".join(tag["term"] for tag in entry["tags"])
    # TODO: re-train with cs.pl in mind
    if pred and "cs.se" not in label.lower() and "cs.pl" not in label.lower():
        ...
        description = (
            ...
            + f"score: {score[1]:.2f}"
            + "<p>"
            + " ".join([y["name"] for y in entry["authors"]])
            + '</p><p><a href="'
            + link
            + '">'
            + link
